...
import networkx
import pickle
G = pickle.load(open('Langley_networks.p'))
...
same_source_colors = []
for e in C[i].edges():
    if C[i].node[e[0]]['heard_from']==0 or C[i].node[e[1]]['heard_from']==0:
        same_source_colors.append('green')
    elif C[i].node[e[0]]['heard_from']==C[i].node[e[1]]['heard_from']:
        same_source_colors.append('blue')
    else:
        same_source_colors.append('red')
...
t = r"$\alpha = -%.2f\pm%.2f$""\n"r"$x_{min} = %d$"%(sizes_fit.alpha, sizes_fit.sigma, sizes_fit.xmin)
t = t.replace('0.','.')

ax.plot(x,y, 'k--', label=t)
text(.05, .05, t, transform = ax.transAxes, horizontalalignment='left')
...
t = r"$\alpha = -%.2f\pm%.2f$""\n"r"$x_{min} = %d$"%(num_children_fit.alpha, num_children_fit.sigma, num_children_fit.xmin)
t = t.replace('0.','.')
